...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const coachesDirectoryPage: MetadataRoute.Sitemap = [
    {
      url: `${baseUrl}/coaches`,
      lastModified: currentDate,
      changeFrequency: 'daily',
  ...
  // Dynamic coach profiles from directory
  let coachProfiles: MetadataRoute.Sitemap = [];
  try {
    const coachesSnapshot = await getDocs(collection(db, 'coach_profiles'));
    coachProfiles = coachesSnapshot.docs
      .filter(doc => doc.data().isPublished === true)
      .map(doc => {
        const data = doc.data();
        const updatedAt = data.updatedAt?.toDate?.() || data.createdAt?.toDate?.() || new Date();
        return {
          url: `${baseUrl}/coaches/${data.slug || doc.id}`,
          lastModified: updatedAt.toISOString(),
          changeFrequency: 'weekly' as const,
          priority: 0.7,
        };
    ...
    console.error('Error fetching coach profiles for sitemap:', error);
  }

  return [...staticPages, ...toolsPages, ...blogPosts, ...coachesDirectoryPage, ...coachProfiles];
}
